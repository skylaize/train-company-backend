...
exports.FREIGHT_DAMPING = exports.STORAGE_FEE_RATE_PER_HOUR = exports.SPREAD = exports.INDEX_MAX = exports.INDEX_MIN = exports.TRADED_CARGO = void 0;
exports.clampIndex = clampIndex;
exports.buyPrice = buyPrice;
exports.sellPrice = sellPrice;
exports.parseHistory = parseHistory;
exports.ensureCargoMarkets = ensureCargoMarkets;
exports.runMarketTick = runMarketTick;
exports.runStorageFees = runStorageFees;
exports.freightMultiplier = freightMultiplier;
exports.getCargoIndexMap = getCargoIndexMap;
...
exports.INDEX_MIN = 0.62;
exports.INDEX_MAX = 1.48;
...
exports.STORAGE_FEE_RATE_PER_HOUR = 0.008;
function clampIndex(v) {
    return Math.max(exports.INDEX_MIN, Math.min(exports.INDEX_MAX, v));
}
function buyPrice(basePrice, index) {
    return Math.max(1, Math.round(basePrice * index * (1 + exports.SPREAD)));
}
function sellPrice(basePrice, index) {
    return Math.max(1, Math.round(basePrice * index * (1 - exports.SPREAD)));
}
...
const HISTORY_EVERY_MS = 5 * 60000;
...
async function ensureCargoMarkets() {
    const existing = await prisma_1.prisma.cargoMarket.findMany({ select: { cargoType: true } });
    const known = new Set(existing.map((e) => e.cargoType));
    for (const cargo of exports.TRADED_CARGO) {
        if (known.has(cargo.cargoType))
            continue;
        ...
        await prisma_1.prisma.cargoMarket.create({
            data: {
...
async function runMarketTick() {
    ...
    for (const m of markets) {
        const def = exports.TRADED_CARGO.find((c) => c.cargoType === m.cargoType);
        const volatility = def?.volatility ?? 1;
        let index = m.index;
        index += MEAN_REVERSION * (1 - index);
        index += NOISE * volatility * gaussian();
        let eventLabel = m.eventLabel;
        let eventUntil = m.eventUntil;
        if (event && event.cargoType === m.cargoType) {
            index += event.shock;
            eventLabel = event.label;
            eventUntil = new Date(now + event.hours * 3600000);
        }
        ...
        const nextHistory = shouldRecord
            ...
            : history;
        await prisma_1.prisma.cargoMarket.update({
...
const STORAGE_EVERY_MS = 10 * 60000;
let lastStorageRun = 0;
async function runStorageFees() {
    const now = Date.now();
    if (now - lastStorageRun < STORAGE_EVERY_MS)
        return;
    const elapsedHours = lastStorageRun === 0 ? STORAGE_EVERY_MS / 3600000 : (now - lastStorageRun) / 3600000;
    lastStorageRun = now;
    const lots = await prisma_1.prisma.stockLot.findMany({ where: { quantity: { gt: 0 } } });
    ...
    for (const [companyId, { units, value }] of byCompany) {
        const fee = Math.round(value * exports.STORAGE_FEE_RATE_PER_HOUR * elapsedHours);
        if (fee <= 0)
            continue;
        ...
        await prisma_1.prisma.$transaction([
            prisma_1.prisma.company.update({ where: { id: companyId }, data: { balance: { decrement: fee } } }),
...
/* ============================================================
   ...
   ============================================================ */
exports.FREIGHT_DAMPING = 0.5;
function freightMultiplier(index) {
    return 1 + exports.FREIGHT_DAMPING * (index - 1);
}
async function getCargoIndexMap() {
    const markets = await prisma_1.prisma.cargoMarket.findMany({ select: { cargoType: true, index: true } });
    return new Map(markets.map((m) => [m.cargoType, m.index]));
}
