...
const stripe = secretKey ? new stripe_1.default(secretKey) : null;
function billingEnabled() {
    return Boolean(stripe && priceId && webhookSecret);
}
async function getBillingStatus(_req, res) {
        ...
        if (companyId && session.payment_status === "paid") {
            ...
            if (company && !company.isPremium) {
                await prisma_1.prisma.$transaction([
                    prisma_1.prisma.company.update({ where: { id: company.id }, data: { isPremium: true } }),
