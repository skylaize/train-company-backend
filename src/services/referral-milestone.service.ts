...
export const QUALIFYING_GRADE_ID = 1;

export const REFERRAL_MILESTONES = [
  { count: 3, reward: "depot" as const, label: "Un emplacement de dépôt supplémentaire" },
  { count: 5, reward: "title" as const, label: "Titre « Recruteur du rail »" },
  { count: 10, reward: "express" as const, label: "Une rame Express offerte" },
];

export const SPONSOR_TITLE = "Recruteur du rail"; // « Chef de réseau » est déjà un grade de carrière : deux sens pour un nom embrouille
...
const QUALIFIED_TTL_MS = 60_000;
let qualifiedCache: { at: number; map: Map<string, number> } | null = null;

/* Filleuls qualifiés par parrain : sert au calcul des paliers et à l'affichage
   de la progression côté joueur. */
export async function countQualifiedReferrals(fresh = false) {
  ...
  const gradeById = new Map<string, number>(rows.map((r) => [r.id, r.gradeId]));
  ...
  (referred as Array<{ id: string; referredById: string | null }>).forEach((r) => {
    if (!r.referredById) return;
    if ((gradeById.get(r.id) ?? 0) >= QUALIFYING_GRADE_ID) {
...
export async function processReferralMilestones() {
  ...
  for (const [sponsorId, count] of qualified) {
    ...
    for (const milestone of REFERRAL_MILESTONES) {
      if (count < milestone.count || granted >= milestone.count) continue;

      if (milestone.reward === "depot") {
        maxTrains += 1;
        await prisma.company.update({ where: { id: sponsorId }, data: { maxTrains } });
      }
